...
function IsThisCourseValid(school_id,iCurrentCourseID,SelectedCourseIDs,SelectedCourseScores) 
{
	...
	for(let j=0;j<pCourseIDs.length;j++)	
		{
			PreReqForCourse2 = SQLike.q({
					SelectDistinct: ['SRC_COURSE'],
				   From: DBPreReq ,
					Where: function(){return (this.TGT_COURSE == pCourseIDs[j]) && (this.REQ=='COURSE_PREREQUISITES')}
			   });
			for(let k=0;k<PreReqForCourse2.length;k++)	
			{
				pCourseIDs.push(PreReqForCourse2[k].SRC_COURSE);
...
*/	
	...
	/*
	for(let m=0;m<iLookBackCount;m++)
	{
	...
	for(let i=0;i<RecordsForEntireGrid.length;i++)	
	{
		...
		else
		{
			...
			{	
				...
				iValid = IsThisCourseValid(school_id,iCurrentCourseID,SelectedCourseIDs,SelectedCourseScores);
