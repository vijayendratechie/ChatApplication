...
app.post("/get_cs_link_url",function(req,res)
{
	var cs_link = null;
	school_id = Custom_Trim(req.body.iSchool_ID);
	var err = null;
	db.query("SELECT DISTINCT CS_LINK FROM school WHERE id=?",[school_id],function(err,result)
	{
		if(err)
		{
			console.log("Error while retreiving course link: " + err);
		}
		else
		{
			cs_link = result[0].CS_LINK;
			res.send(cs_link);
		}
	});
});
	...
	for(let i=0;i<RecordsForEntireGrid.length;i++)	
	{
		...
		else
		{
			...
			{	
				...
				//console.log(school_id+iCurrentCourseID+iCurrentCourseGrade+iCurrentTrack);
