...
var year_sequence = [{9 : 1},{10 : 2},{11 : 3},{12 : 4}];
...
app.get("/GetAllCourses",function(req,res)
{
	...
	for(let i=0;i<req.query.arrayOfAllGridInfo.length;i++)	{
			...
			db.query("SELECT `COURSE_ID`,`NAME`,`MANDATORY` FROM subjects WHERE SCHOOL_ID = 323 AND SUBJECT = ? AND LGRADE <= ? AND HGRADE >= ? AND COURSE_ID NOT IN (SELECT DISTINCT CORE FROM prerequisites)",[req.query.arrayOfAllGridInfo[i].SUBJECT,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE],function(err,result)
			{				
				AvailableSubjects = [];
				...
				else
				{
					...
					for(var j=0;j<result.length;j++)
					{
						...
						if(result[j].COURSE_ID == InputSelectedSubjectID)
						{
							SelectedSubjectName = result[j].NAME;
							SelectedSubjectID = result[j].COURSE_ID;							
						}
	...
	for(let i=0;i<ListOfAllSelected.length;i++)	{
		
			db.query("SELECT DISTINCT subjects.course_id, subjects.name, subjects.lgrade, subjects.hgrade, subjects.subject FROM prerequisites join subjects on prerequisites.school_id = subjects.school_id AND prerequisites.REQ = 'COURSE_PREREQUISITES' AND prerequisites.core = subjects.COURSE_ID AND prerequisites.SCHOOL_ID = 323 AND LGRADE >= ? AND HGRADE > ? and prerequisites.course in (?)",[ListOfAllSelected[i].GRADE,ListOfAllSelected[i].GRADE,ListOfAllSelected[i].SELECTEDID],function(err,result)
			{	
				if(err)
				{
					console.log("Error while retreiving dependant courses of all grid with no value selected " + err);
				}
				else
				{	
					for(var j=0;j<result.length;j++)
					{
						iAlreadyAdded = false
						for(let k=0;k<RecordsForEntireGrid.length;k++)	{
							if(RecordsForEntireGrid[k].SUBJECT == result[j].subject && RecordsForEntireGrid[k].GRADE > result[j].lgrade) {
								if(RecordsForEntireGrid[k].SELECTEDID == result[j].COURSE_ID) {
									iAlreadyAdded = true
								}
								else {								
									for (let l=0;l<RecordsForEntireGrid[k].AVAILABLECOURSES.length;l++)	{
										if(RecordsForEntireGrid[k].AVAILABLECOURSES[l] == result[j].COURSE_ID) {
											iAlreadyAdded = true
											break;
										}
									}
								...
								if(iAlreadyAdded = false) {
										RecordsForEntireGrid[k].AVAILABLECOURSES[l].push(
									{	SUBTRACK_ID : result[j].COURSE_ID,
										SUBTRACK_NAME : result[j].NAME
									})	;
								}						
