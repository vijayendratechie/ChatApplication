...
db.query("SELECT distinct id FROM school order by id",function(err,result_school_ids)
{
	...
	else
	{
		for(let j=0;j<result_school_ids.length;j++)
		{
			...
			//db.query("SELECT DISTINCT TRACK_NAME AS SUBJECT FROM tracks where school_id = ? and track_id = 'English' order by track_seq  ",[result_school_ids[j].id],function(err,result)
...
app.post("/GetAllCourses",function(req,res)
{
	...
	for(let i=0;i<req.body.arrayOfAllGridInfo.length;i++)
	{
		...
		var oRecordsForEntireGrid = [];
		oRecordsForEntireGrid.push(RecordsForEntireGrid);
		oRecordsForEntireGrid.push(oSchoolGradeReqArr);
		oRecordsForEntireGrid.push(oStateGradeReqArr);
		console.clear() ;
		myprint(oSchoolGradeReqArr);
		myprint(oStateGradeReqArr);
		res.send(oRecordsForEntireGrid);
});
	...
	for(let k=0;k<All_SUBJECTS.length;k++)	
	{
		...
		for(let i=0;i<RecordsForEntireGrid.length;i++)	
		{
				if( Custom_Trim(All_SUBJECTS[k].SUBJECT) == RecordsForEntireGrid[i].SUBJECT)
				{
					...
					{
						iConsiderReqMet = true;
						if(RecordsForEntireGrid[i].AVAILABLECOURSES[j].COURSE_ID != 0 && DBThisSchool[0].consider_score == true && 
							(GetScoreValue(school_id, RecordsForEntireGrid[i].AVAILABLECOURSES[j].COURSE_SCORE) < GetScoreValue(school_id,minRequiredScore)) && RecordsForEntireGrid[i].AVAILABLECOURSES[j].COURSE_SCORE != "NA")
						{							
