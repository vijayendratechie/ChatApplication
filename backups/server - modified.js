...
//custom print to print json elements on separate lines
...
app.get("/GetAllCourses2",function(req,res)
{
	...
	for(let i=0;i<req.query.arrayOfAllGridInfo.length;i++)	{
			...
			{		
				console.time('100-1');
				db.query("SELECT `COURSE_ID`,`NAME`,`MANDATORY` FROM subjects WHERE SCHOOL_ID = ? AND replace(replace(SUBJECT,' ',''),'.','') = ? AND LGRADE <= ? AND HGRADE >= ?",[ischool_id,req.query.arrayOfAllGridInfo[i].SUBJECT,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE],function(err,result)
			...
			{
				console.time('100-2');
				db.query("SELECT `COURSE_ID`,`NAME`,`MANDATORY` FROM subjects WHERE SCHOOL_ID = ? AND replace(replace(SUBJECT,' ',''),'.','') = ? AND LGRADE <= ? AND HGRADE >= ? AND (COURSE_ID NOT IN (SELECT DISTINCT IFNULL(CORE,'-100') FROM prerequisites  WHERE REQ = 'COURSE_PREREQUISITES'))",[ischool_id,req.query.arrayOfAllGridInfo[i].SUBJECT,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE],function(err,result)
		...
		console.log("Backend Final Array is : 5555: "+ JSON.stringify(RecordsForEntireGrid));
