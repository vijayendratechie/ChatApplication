...
app.get("/GetAllCourses",function(req,res)
{
	...
	for(let i=0;i<req.query.arrayOfAllGridInfo.length;i++)	{
			...
			db.query("SELECT `COURSE_ID`,`NAME`,`MANDATORY` FROM subjects WHERE SCHOOL_ID = 323 AND SUBJECT = ? AND LGRADE <= ? AND HGRADE >= ? AND (COURSE_ID NOT IN (SELECT DISTINCT CORE FROM prerequisites) OR LGRADE=?)",[req.query.arrayOfAllGridInfo[i].SUBJECT,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE],function(err,result)
			{				
				AvailableSubjects = [];
