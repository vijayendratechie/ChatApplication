...
passport.use(new localStrategy({
	...
	function(req,username,password,done)
	{
		...
		const resultsetB = db.query("SELECT id AS user_id from student WHERE email = ?",[username]);
		{
...
const resultsetA = db.query("(SELECT DISTINCT `SUBJECT` FROM subjects LIMIT 2) UNION SELECT `SUBJECT` FROM subjects WHERE SUBJECT = 'MATHS'");
assert(resultsetA);
	...
	//console.log(resultsetA);
	{
		
		{
			...
			//console.log(resultsetA);
			{
				...
				{
					//console.log(result);
					res.render("home",{display_name : res.locals.display_name, data : result, year: year, subject : subject,scenario_name : resultsetB});
				}
			};			
		}
	};	 	
...
app.get("/sent_fav_flag",function(req,res)
{
	...
	{
		const result1 = db.query("SELECT scenario_name FROM scenario_name WHERE fav = 1 AND student_id = ? AND status != 'delete'",[req.user.user_id]);
		assert(resultsetA);
		{
...
app.get("/GetAllCourses",function(req,res)
{
	...
	for(let i=0;i<req.query.arrayOfAllGridInfo.length;i++)	{
			...
			const result = db.query("SELECT `COURSE_ID`,`NAME`,`MANDATORY` FROM subjects WHERE SCHOOL_ID = 323 AND SUBJECT = ? AND LGRADE <= ? AND HGRADE >= ? AND (COURSE_ID NOT IN (SELECT DISTINCT CORE FROM prerequisites) OR LGRADE=?)",[req.query.arrayOfAllGridInfo[i].SUBJECT,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE,req.query.arrayOfAllGridInfo[i].GRADE]);
			assert(result);
