...
function IsThisCourseValid(school_id,iCurrentCourseID,SelectedCourseIDs,SelectedCourseScores,iCurrentCourseSubject,iCurrentTrack,SelectedCourseTracks) 
{
	...
	{		
		//iMasterCondition = PreReqForCourse[i].MASTER_CONDITION;
	...
	//If this course a sibling courses of already selected, if yes, mark is invalid
	
	SiblingCourses = [];
	findSiblingCourses(SelectedCourseIDs, SiblingCourses,SelectedCourseTracks);
	for(let k=0;k<SiblingCourses.length;k++)	
	{
		if(SiblingCourses[k] == iCurrentCourseID)
		{
			return false;
		}
	}
...
/*
	//Is this course lower than already selected course (direct or cascade down), collect all prior courses recruisively and check again current course id
	pCourseIDsCurrentCourse = [];
	var PreReqForCourse2CurrentCourse = [];

	PreReqForCourse2CurrentCourse = SQLike.q({
			SelectDistinct: ['SRC_COURSE'],
			From: DBPreReq ,
			Where: function(){return (this.TGT_COURSE == iCurrentCourseID) && (this.REQ=='COURSE_PREREQUISITES')}
		});
	for(let k=0;k<PreReqForCourse2CurrentCourse.length;k++)	
	{
		pCourseIDsCurrentCourse.push(PreReqForCourse2CurrentCourse[k].SRC_COURSE);
	}

	//we will iterate n times to gather as much recrusively for dependant courses
	AddPriorCoursesToArray(pCourseIDsCurrentCourse, PreReqForCourse2CurrentCourse);

	//now we have list of all prior courses for given selected course, if any of the prior courses matches to current course; remove/mark as valid=no by returning false for this course.
	for(let j=0;j<pCourseIDs.length;j++)	
	{
		for(let n=0;n<pCourseIDsCurrentCourse.length;n++)
		{
			if(pCourseIDs[j] == pCourseIDsCurrentCourse[n])
			{
				return false;
			}
		}
...
//Find Subling Courses to given COURSE
function findSiblingCourses(SelectedCourseIDs, SiblingCourses,SelectedCourseTracks)
{	
	for(let m=0;m<SelectedCourseIDs.length;m++)
	{
				
		SharedPreReq_IDs = SQLike.q({
						SelectDistinct: ['TGT_COURSE','REQ_ID'],
					   From: DBPreReq ,
						Where: function(){return (this.SRC_COURSE == SelectedCourseIDs[m]) && (this.REQ=='COURSE_PREREQUISITES')}
									   });
		
		for(let k=0;k<SharedPreReq_IDs.length;k++)	
		{
			
			TrackOfSelectedCourse = SQLike.q({
				SelectDistinct: ['SUBJECT'],
			   From: DBCourses ,
				Where: function(){return (this.COURSE_ID == SharedPreReq_IDs[k].TGT_COURSE)}
		   });
			
			if(TrackOfSelectedCourse[0].SUBJECT == SelectedCourseTracks[m])
			{
				SiblingCourse2 = SQLike.q({
								SelectDistinct: ['SRC_COURSE'],
							   From: DBPreReq ,
								Where: function(){return (this.REQ_ID == SharedPreReq_IDs[k].REQ_ID) && (this.REQ=='COURSE_PREREQUISITES')}
						   });
				for(let l=0;l<SiblingCourse2.length;l++)	
				{		   
					if(SiblingCourse2[l].SRC_COURSE != SelectedCourseIDs[m])
					{
						SiblingCourses.push(SiblingCourse2[l].SRC_COURSE);
					}
				}
			}
		}
...
function AddPriorCoursesToArray(pCourseIDs, PreReqForCourse2) 
{
	...
	//while(index <= num)
	{
		PreReqForCourse2 = SQLike.q({
					SelectDistinct: ['SRC_COURSE'],
				   From: DBPreReq ,
					Where: function(){return (this.TGT_COURSE == pCourseIDs[index]) && (this.REQ=='COURSE_PREREQUISITES')}
			   });
		for(let k=0;k<PreReqForCourse2.length;k++)	
		{
			pCourseIDs.push(PreReqForCourse2[k].SRC_COURSE);
	...
	for(let i=0;i<RecordsForEntireGrid.length;i++)	
	{
		...
		else
		{
			...
			{	
				...
				iValid = IsThisCourseValid(school_id,iCurrentCourseID,SelectedCourseIDs,SelectedCourseScores,iCurrentCourseGrade,iCurrentTrack,SelectedCourseTracks);
				/*for(let d=0;d<ValidCourseSoFar.length;d++)	
				{
					if(ValidCourseSoFar[d] == iCurrentCourseID)
					{
						isPreValidated = true;
						RecordsForEntireGrid[i].AVAILABLECOURSES[j].VALID = ValidCourseSoFarStatus[d];
						iValid = ValidCourseSoFarStatus[d];
					}
				}
				if(isPreValidated == false)
				{
					iValid = IsThisCourseValid(school_id,iCurrentCourseID,SelectedCourseIDs,SelectedCourseScores,iCurrentCourseGrade,iCurrentTrack,SelectedCourseTracks);
					ValidCourseSoFar.push(iCurrentCourseID);
					ValidCourseSoFarStatus.push(iValid);
				}
				*/
...
app.post("/GetAllCoursesForFirstTime",function(req,res)
{
	var iSchoolID = Custom_Trim(req.body.iSchool_ID);
	DBPreReq = GetDBPreReqForSchool(iSchoolID);
	var RecordsForEntireGrid = [];	
	
	var AllYears = GetYearsForSchool(iSchoolID);
	var AllTracks =  GetSubjectsForSchool(iSchoolID);
	var NoOfTracks = AllTracks.length;
	var NoOfYears = AllYears.length;
	
	DBCourses = GetDBCoursesForSchool(iSchoolID);
	...
	var AlredyFailedValidationCourses = [];
	var NewSelectedCourses = [];
	var UISelectedCourseID = null;
	
	var iSelected = false;
	
	for(let ii=0;ii<AllTracks.length;ii++)
	{
		for(let jj=0;jj<AllYears.length;jj++)
		{
			N_PopulateDefaultCoursesForThisYearAndTrack(RecordsForEntireGrid,jj,AllYears[jj].YEAR,AllTracks[ii].SUBJECT)
		}
	}
	res.send(RecordsForEntireGrid);
})

function N_PopulateDefaultCoursesForThisYearAndTrack(RecordsForEntireGrid,jj,iYEAR,iSUBJECT)
{
	var AvailableCourses = [];	
	var AlredyFailedValidationCourses = [];
		if(jj == 0)
		{
			...
			result =[];
			result = SQLike.q({
					Select: ['COURSE_ID','NAME','MANDATORY','CREDIT','LEVEL'],
				   From: DBCourses ,
					Where: function(){return (this.LGRADE  <= iYEAR) && (this.HGRADE >= iYEAR) && (this.SUBJECT ==  iSUBJECT)},
					OrderBy: ['COURSE_ID']
			   });
			   
			for(var k=0;k<result.length;k++)
			{						
				if(result[k].MANDATORY == "YES")
				{					
					AvailableCourses =[];
					AvailableCourses.push(
					{	COURSE_ID : result[k].COURSE_ID,
						COURSE_NAME : result[k].NAME,
						COURSE_SCORE : NAScore,
						VALID : true,
						MANDATORY : true,
						SELECTED : true,
						CREDIT : result[k].CREDIT,
						LEVEL : result[k].LEVEL
					})	;							 
					break;
				}
				
				if(result[k].COURSE_ID == 0)
				{
					AvailableCourses.push(
					{	COURSE_ID : result[k].COURSE_ID,
						COURSE_NAME : result[k].NAME,
		...
		{
			//isthiscoursevalid(prior selection, is this dependant course, 
			...
			for(var k=0;k<result.length;k++)
			{						
				for(var m=0;m<AlredyFailedValidationCourses.length;m++)
				{
					if (AlredyFailedValidationCourses[m] == result[k].COURSE_ID)
					{
						continue;
					}
				}

				//DoesThisCourseRequirePreReq
				PreReqCourses = SQLike.q({
					SelectDistinct: ['TGT_COURSE'],
				   From: DBPreReq ,
					Where: function(){return (this.REQ=='COURSE_PREREQUISITES') && (this.TGT_COURSE == result[k].COURSE_ID)}			
			   });
				if (PreReqCourses.length > 0)
				{
					AlredyFailedValidationCourses.push(result[k].COURSE_ID);
					continue;
				}
...
app.post("/GetCoursesUponChange",function(req,res)
{
	var iSchoolID = Custom_Trim(req.body.iSchool_ID);
	DBPreReq = GetDBPreReqForSchool(iSchoolID);
	var RecordsForEntireGrid = [];	
	...
	var NewSelectedCourses = [];
	var UISelectedCourseID = null;
	var iSubjectChanged = null;
	var priorSelectedCourseID = null;
	var result = [];
	var iSelected = false; 
	var NextEligibleCourses = [];
	var tempSelectedID = [];

	for(let i=0;i<AllTracks.length;i++)
	{
		if(AllTracks[i].SUBJECT == req.body.arrayOfAllGridInfo[0].SUBJECT)
		{
			iSubjectChanged = i;
			break;
		}
	}	
	for(let i=iSubjectChanged;i<iSubjectChanged+1;i++)
	{
		var AlredyFailedValidationCourses = [];
		
		for(let j=0;j<AllYears.length;j++)
		{
			iSelectedCourseID = null;
			tempSelectedID = [];
			NextEligibleCourses = [];
			if(req.body.arrayOfAllGridInfo[j].SelectedSubjectID == null)
			{
				iSelectedCourseID = priorSelectedCourseID;
			}
			else
			{
				iSelectedCourseID = req.body.arrayOfAllGridInfo[j].SelectedSubjectID;
			}
			...
			if(j==0)
			{
				N_PopulateDefaultCoursesForThisYearAndTrack(RecordsForEntireGrid,j,AllYears[j].YEAR,AllTracks[i].SUBJECT);

				for(let kk=0;kk<RecordsForEntireGrid[0].AvailableCourses.length;kk++)
				{
					if(RecordsForEntireGrid[0].AvailableCourses[i].COURSE_ID == iSelectedCourseID)
					{
						RecordsForEntireGrid[0].AvailableCourses[i].SELECTED = true;
						priorSelectedCourseID = iSelectedCourseID;
			...
			else
			{
				...
				else
				{
					*/
					...
					AvailableCourses = [];
					iSelectedCourseID = iSelectedCourseID.substring(0,iSelectedCourseID.length-1);
					tempSelectedID.push(iSelectedCourseID);
					N_GetEligibleCourses(AllYears[j].YEAR,iSchoolID,NextEligibleCourses,tempSelectedID);
					for(let ii=0;ii<NextEligibleCourses.length;ii++)
					{								
						for(var is=0;is<result_scores.length;is++)
						{
							iSelected = false;
							if(iSelectedCourseID == NextEligibleCourses[ii] + result_scores[is][0])
							{
								iSelected = true;
							...
							AvailableCourses.push(
							{	COURSE_ID : NextEligibleCourses[ii].COURSE_ID,
								COURSE_NAME : 'test',
								COURSE_SCORE : result_scores[is][0],
								VALID : true,
								MANDATORY : false,
					...
					RecordsForEntireGrid.push(
						{						
							...
							SUBJECT : req.body.arrayOfAllGridInfo[0].SUBJECT,													
							AVAILABLECOURSES : 	AvailableCourses							
						});
				//}
...
function N_GetEligibleCourses(iYear,iSchoolID,NextEligibleCourses,SelectedCourseIDsForThisTrack)
{
	DBPreReq = GetDBPreReqForSchool(iSchoolID);
	var NextCourses = [];
	var NextReq_IDs = [];
	...
	NextCourses = SQLike.q({
		SelectDistinct: ['*'],
	   From: DBPreReq ,
		Where: function(){return (this.REQ=='COURSE_PREREQUISITES') && (this.SRC_COURSE == '2210')},
		OrderBy: ['REQ_ID','SUB_REQ_ID']			
	});
	
	for(let i=0;i<NextCourses.length;i++)	
	{
		iMasterCondition = NextCourses[i].MASTER_CONDITION;
		iMasterCondition = iMasterCondition.split('OR').join(' || ');
		iMasterCondition = iMasterCondition.split('AND').join(' && ');
		
		for (let j=0;j<SelectedCourseIDsForThisTrack.length;j++)
		{
			if(SelectedCourseIDsForThisTrack[j] == NextCourses[i].SRC_COURSE)
			{
				iMasterCondition = iMasterCondition.replace (NextCourses[i].SUB_REQ_ID, 'true');
			}
		}
		for (let j=0;j<iCascadeConditions;j++)
		{
		...
		if (eval(iMasterCondition) == true) 
		{
			NextEligibleCourses.push({
				COURSE_ID : 	NextCourses[i].TGT_COURSE
			});
		}
	}
}
...
app.post("/GetAllCoursesNew",function(req,res)
{
	var iSchoolID = Custom_Trim(req.body.iSchool_ID);
	var RecordsForEntireGrid = [];	
	var TheseYears = GetYearsForSchool(iSchoolID);
	var TheseTracks =  GetSubjectsForSchool(iSchoolID);
	var iFirstYear = TheseYears[0].YEAR;
	...
	PopulateFirstGrade(iFirstYear,iSchoolID,TheseTracks,RecordsForEntireGrid,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,CurrentUISelections);
	//FirstSelecttion = 
	for(let i=0;i<TheseTracks.length;i++)
	{
		var tempSelections = SelectedCourseIDsForThisTrack[i];
		//var tempSelections = SelectedCourseIDsForThisTrack[i];
		//var temp = SelectedCourseIDsForThisTrack[i];
		//SelectedCourseIDsForThisTrack[i] = [];
		for(let j=1;j<TheseYears.length;j++)
		{
			
			
			NewSelectedCourses = PopulateSubSequentGradeForThisTrack(TheseYears[j].YEAR,iSchoolID,TheseTracks[i].SUBJECT,RecordsForEntireGrid,tempSelections,SelectedCourseIDsEntireList,CurrentUISelections);
			if(NewSelectedCourses.length >0)
			{
				tempSelections = NewSelectedCourses;
			}
		}
	}
...
{
	for(let i=0;i<CurrentUISelections.length;i++)
	{
...
function PopulateFirstGrade(iFirstYear,iSchoolID,TheseTracks,RecordsForEntireGrid,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,CurrentUISelections)
{
	var AvailableCourses= [];
	...
	{
		SelectedCourseIDsForThisTrack[i] = [];
		if(TheseTracks[i].SUBJECT=='PhysEd')
		{
			console.log(TheseTracks[i].SUBJECT);
		
		}
		UiSelectedCourse = GiveUISelectedCourseForThisGradeAndTrack(CurrentUISelections, iFirstYear,TheseTracks[i].SUBJECT );
		
		result =[];
		result = SQLike.q({
				Select: ['*'],
			   From: DBCourses ,
				Where: function(){return (this.LGRADE  <= iFirstYear) && (this.HGRADE >= iFirstYear) && (this.SUBJECT ==  TheseTracks[i].SUBJECT)},
				OrderBy: ['COURSE_ID']
		   });
		...
		{						
			...
			if(UiSelectedCourse == result[j].COURSE_ID + ' - ' + NAScore)
			{
				...
				SelectedCourseIDsForThisTrack[i].push(result[j].COURSE_ID);
				SelectedCourseIDsEntireList.push({
					COURSE_ID : result[j].COURSE_ID,
					SCORE : NAScore,
					TRACK :  TheseTracks[i].SUBJECT,
					YEAR :  iFirstYear
				});
			}
			if(result[j].COURSE_ID == 0)
			...
			{						
				GRADE :  iFirstYear,
				SUBJECT : TheseTracks[i].SUBJECT,													
				AVAILABLECOURSES : 	AvailableCourses							
			});
	}	
}

function PopulateSubSequentGradeForThisTrack(iYear,iSchoolID,iTrack,RecordsForEntireGrid,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,CurrentUISelections)
{
	var AvailableCourses= [];
	var NextEligibleCourses = [];
	var NewSelectedCourse = [];
	var NoCourseHasAPreReq = false;

	if(iTrack=='eMathematics')
	{
		console.log(iTrack.SUBJECT);
	
	}
	UiSelectedCourse = GiveUISelectedCourseForThisGradeAndTrack(CurrentUISelections, iYear,iTrack );
	
	
	
	//if(SelectedCourseIDsForThisTrack.length > 0 && NoCourseHasAPreReq == true)
		if(SelectedCourseIDsForThisTrack.length > 0 )
	{
		GetEligibleCourses(iYear,iSchoolID,iTrack,NextEligibleCourses,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,NewSelectedCourse);
		if(NextEligibleCourses.length > 0)
		{
			SelectedCourseIDsForThisTrack = [];
		}

		AddCourses(iYear,iTrack,NextEligibleCourses,UiSelectedCourse,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,RecordsForEntireGrid,NewSelectedCourse);

		
	}
	else
	{
		//populate default courses for this grade
		result = GiveMeCoursesWithoutPreReq(iYear,iTrack,school_id,NoCourseHasAPreReq) ;
	   AddCourses(iYear,iTrack,result,UiSelectedCourse,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,RecordsForEntireGrid,NewSelectedCourse);
	}

	return NewSelectedCourse;
}

function GetEligibleCourses(iYear,iSchoolID,ThisTrack,NextEligibleCourses,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,NewSelectedCourse)
{
	DBPreReq = GetDBPreReqForSchool(iSchoolID);
	var NextCourses = [];
	var NextReq_IDs = [];
	...
	NextCourses = SQLike.q({
		SelectDistinct: ['*'],
	   From: DBPreReq ,
		Where: function(){return (this.REQ=='COURSE_PREREQUISITES')},
		OrderBy: ['REQ_ID','SUB_REQ_ID']			
	});
...
function addEmptyCourse(iYear,iTrack,UiSelectedCourse,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,RecordsForEntireGrid,NewSelectedCourse)
{
	var iSelected = false;
	result = SQLike.q({
		Select: ['*'],
	   From: DBCourses ,
		Where: function(){return (this.COURSE_ID == 0) && (this.SUBJECT ==  iTrack) }
   });
   for(var j=0;j<result.length;j++)
   {
	// if placeholder course, push it with NA score requirement.
...
function AddCourses(iYear,iTrack,resultIDs,UiSelectedCourse,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,RecordsForEntireGrid,NewSelectedCourse)
{
	
			
			AvailableCourses = [];
			
			addEmptyCourse(iYear,iTrack,UiSelectedCourse,SelectedCourseIDsForThisTrack,SelectedCourseIDsEntireList,RecordsForEntireGrid);
			...
			{						
				...
				result = SQLike.q({
					Select: ['*'],
				   From: DBCourses ,
					Where: function(){return (this.COURSE_ID == resultIDs[i].COURSE_ID) && (this.SUBJECT ==  iTrack)}
			   });
				...
				if(result[j].COURSE_ID != 0)
				{
					for(var is=0;is<result_scores.length;is++)
					{
						...
						if(UiSelectedCourse == result[j].COURSE_ID + result_scores[is][0])
						{
							...
							NewSelectedCourse.push(result[j].COURSE_ID);
...
//Validate if current course requires any pre-req courses
function GiveMeCoursesWithoutPreReq(iYear,iTrack,school_id,NoCourseHasAPreReq) 
{
	var result = [];
	var iTotalCourses = null;
	var iCoursesWhichReqPreReq = null;
	DBCourses = GetDBCoursesForSchool(school_id);
	DBPreReq = GetDBPreReqForSchool(school_id);
	var iRequire = true;
	eligible_courses =[];		
	eligible_courses = SQLike.q({
	Select: ['*'],
	   From: DBCourses ,
		Where: function(){return (this.LGRADE  <= iYear) && (this.HGRADE >= iYear) && (this.SUBJECT ==  iTrack)},
		OrderBy: ['COURSE_ID']
   });
	...
	for(let i=0;i<eligible_courses.length;i++)	
	{
		iTotalCourses++;
		iRequire = false;
		if(eligible_courses[i].COURSE_ID != 0)
		{		
			for(let j=0;j<PreReqCourses.length;j++)	
			{
				if(eligible_courses[i].COURSE_ID == PreReqCourses[j].TGT_COURSE)
				{	
					iCoursesWhichReqPreReq++;
					iRequire = true;
					break;
				}
			}
		}
		if(iRequire == false && eligible_courses[i].COURSE_ID != 0)
		{
			result.push (eligible_courses[i]);
		}
	}
	
	if(iCoursesWhichReqPreReq == 0)
	{
		NoCourseHasAPreReq = true;
	}
	return result;
}